...
extension OTel.Tracer: Instrument {
    public func extract<Carrier, Extract>(
        _ carrier: Carrier,
        into serviceContext: inout ServiceContext,
        using extractor: Extract
    ) where Carrier == Extract.Carrier, Extract: Extractor {
        do {
            serviceContext.spanContext = try propagator.extractSpanContext(from: carrier, using: extractor)
        } catch {
    ...
    public func inject<Carrier, Inject>(
        ...
        guard let spanContext = serviceContext.spanContext else { return }
...
extension OTel.Tracer: Tracer {
    ...
    public func startSpan<Instant: TracerInstant>(
        ...
        let parentServiceContext = context()
        var childServiceContext = parentServiceContext

        let traceID: OTel.TraceID
        let traceState: OTel.TraceState?
        let spanID = idGenerator.generateSpanID()

        if let parentSpanContext = parentServiceContext.spanContext {
        ...
        let samplingResult = sampler.makeSamplingDecision(
            ...
            attributes: [:],
            links: [],
            parentServiceContext: parentServiceContext
        )
        let traceFlags: OTel.TraceFlags = samplingResult.decision == .recordAndSample ? .sampled : []
        let spanContext = OTel.SpanContext(
            traceID: traceID,
            spanID: spanID,
            parentSpanID: parentServiceContext.spanContext?.spanID,
            traceFlags: traceFlags,
            traceState: traceState,
            isRemote: false
        )
        childServiceContext.spanContext = spanContext
