...
final class TracerTests: XCTestCase {
    ...
    func test_startingChildSpan_respectsSamplingDecision() throws {
        ...
        serviceContext.spanContext = parentSpanContext
        let span = tracer.startSpan(#function, context: serviceContext)
    ...
    func test_extractsSpanContextUsingTheConfiguredPropagator() throws {
        ...
        var serviceContext = ServiceContext.topLevel
        tracer.extract(headers, into: &serviceContext, using: DictionaryExtractor())

        let spanContext = try XCTUnwrap(serviceContext.spanContext)
